        ...
        <div class="bradcumbContent">
            ...
            <%
                ...
                try
                {
                    ...
                    ResultSet rs1=null;
                    rs=s.executeQuery("select * from playlist_songs where email='"+email+"' and playlistname='"+playlist+"';");
                    while(rs.next())
                    {
                        sid=rs.getString(3);
                        song_here.add(Integer.parseInt(sid));
                        rs1=s1.executeQuery("select songname,songphotoloc from songs where songid="+sid);
                        while(rs1.next())
                        {
                            song_name.add(rs1.getString(1));
                            song_photo.add(rs1.getString(2));
                        }
                    }
                       ...
                       <div class="single-new-item d-flex align-items-center justify-content-between wow fadeInUp" data-wow-delay="100ms">
                            <div class="first-part d-flex align-items-center">
                                ...
                                <div class="content-" style="border-bottom: 1px grey;">
                                    <h2 style="font-size: 18px;text-transform: uppercase;letter-spacing: 10px"><a href="song_page.jsp?email=<%=email%>&&songid=<%=song_here.get(i)%>"><%=song_name.get(i)%></a>
                                    <a href="deleteplaylistsong.jsp?email=<%=email%>&&playlist=<%=playlist%>&&sid=<%=song_here.get(i)%>">
                        <button type="submit" class="fa fa-trash" style="background: transparent;border: 0px;font-size: 1.6em; margin-bottom: 10px; color: red;cursor:pointer"></button></a>
                                    </h2>
                                </div>
                           </div>
                        </div>
