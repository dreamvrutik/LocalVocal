        ...
        <div class="bradcumbContent">
            ...
            <%
                ...
                try
                {
                    ...
                    ResultSet rs1=null;
                    aid=request.getParameter("albumid");
                    ploc+=aid;
                    ploc+=".png";
                    ...
                    rs=s.executeQuery("select count from albums where albid="+aid);
                    int c=0;
                    while(rs.next())
                    {
                        c=rs.getInt(1);
                    ...
                    c+=1;
                    s.execute("update albums set count="+c+" where albid="+aid+";");
                    int flag=0;
                    rs=s.executeQuery("select count(*) from album_song where albid="+aid+";");
                    ...
                    if(flag==1)
                    {
                        rs=s.executeQuery("select songid from album_song where albid="+aid+";");
                        System.out.println(2);
                        while(rs.next())
                        {
                            sid=rs.getString(1);
                            song_here.add(Integer.parseInt(sid));
                            String pp="song_photo/"+sid+".png";
                            song_photo.add(pp);
                            rs1=s1.executeQuery("select songname from songs where songid="+sid);
                            while(rs1.next())
                            {
