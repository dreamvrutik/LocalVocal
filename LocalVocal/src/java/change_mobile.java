...
public class change_mobile extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            {
                newm=request.getParameter("newmobile");
            }
            if(!newm.equals("")&&newm.length()==10)
            {
                String in="update user_database set mobile='"+newm+"' where email='"+email+"';";
                s.execute(in);
                response.sendRedirect("myprofile.jsp?email="+email);
            }
            else
            {
                response.sendRedirect("myprofile.jsp?email="+email+"&&errcode=4");
            }
        }
        catch(Exception e)
        {
